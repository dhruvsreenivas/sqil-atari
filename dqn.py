...
class QLearner(object):
  ...
  def __init__(
    ...
    if sqil:
      ...
      print('demos loaded')
    if gen_demos:
      load_tf_vars(self.session, 'q_func_chkpt.tf')
      ...
      while len(demos) < 50:
        ...
        while not done:
          obs, act, rew, done, info = self.step_env(demo=True)
          demo.append((obs, act, rew, done))
          if 'ale.lives' in info: # true for breakout, false for pong
            done = info['ale.lives'] == 0
        episode_rewards = get_wrapper_by_name(self.env, "Monitor").get_episode_rewards()
        rtn = episode_rewards[-1]
        print(rtn, len(demos))
        # breakout:
  ...
  def step_env(self, demo=False):
    ...
    self.last_obs, r, done, info = self.env.step(action)
    #self.env.render()
    ...
    if len(episode_rewards) > 100:
      if self.mean_episode_reward > self.best_mean_episode_reward:
        ...
        if self.model_initialized:
          save_tf_vars(self.session, 'q_func_chkpt.tf')
